...
{
    return _midiFile && !_needStop && !_pause;
}
    ...
    while (true)
    {
        ...
        if (_midiFile)
        {
            ...
            for (_currentPosition = 0; _currentPosition < _events.count(); _currentPosition += 1)
            {
                ...
                {
                    _currentTime = qint64(_midiFile->timeFromTick(e->tick()) * 1000.0f);
...
MidiPlayer::Error MidiPlayer::play(QMidiFile *midiFile)
{
    QMutexLocker locker(&_mutex);
    ...
    if (_midiFile != midiFile){
        _maxTime = calcTime(midiFile);
        _midiFile = midiFile;

        emit updateProgress(0, _maxTime);

        _elapsedTime.start();
        _events = _midiFile->events();
        _amendmentTime = 0;
...
QMidiFile *MidiPlayer::currentMidiFile() const
{
    return _midiFile;
}
